...
\begin{aligned}
    &u_{tt}-c^2\Delta u=0 \quad \textrm {for} \quad x \in \mathbb{R}^3, \quad t \in \mathbb{R}_{>0}\\
    &u(x,0)=g(x)\\
    &u_t(x,0)=h(x).\\
\end{aligned}
\end{equation}
...
Firstly, we introduce some notation to simplify our work in the upcoming sections. If we let $x\in \mathbb{R}^n, t>0, r>0$, then we have:

\begin{equation} \label{averageU}
    U(x;r,t)=\dashint_{\partial B(x,r)}u(y,t)dS(y),
\end{equation}

the average of $u(\cdot,t)$ over the sphere $\partial B(x,r)$, as in \cite{Ev}. Similarly, we have the averages of $g$ and $h$ over the same sphere:
...
When we fix $x$, we can regard $U$ as a function of just $r$ and $t$. Note that if $u$ satisfies equation
...
Note that our definition of $U$ from (\ref{averageU}) implies that $u(x,t)=\lim_{r\rightarrow 0^+}U(x;r,t)$, so our definitions of $\tilde{G}, \tilde{H}$ in (\ref{GHdash}),
...
Note that in (\ref{prereq}) we saw that $\int_{\partial B(x,t)}f(y)dS=t^2\int_{\partial B(0,1)} f(x+tz)dS(z)$. We will use this to complete our solution of the
...
to solve the case in three dimensions and then \emph{descend} into two dimensions by letting the $z$-coordinate equal $0$, as in \cite{Ev}. We shall see how this works in the 
next section. 
