...
equation derived in \ref{twodim}. Before we prove the solution, we will discuss
...
Using a similar transformation to the one described in section \ref{prereq}, we
get that
$t^2\dashint_{B(\boldsymbol{x},t)}\frac{g(\boldsymbol{y})}{(t^2-|\boldsymbol{y}-\boldsymbol{x}|^2)^{1/2})}d\boldsymbol{y}$
is equal to
$t\dashint_{B(0,1)}\frac{g(\boldsymbol{x}+t\boldsymbol{z})}{(1-|\boldsymbol{z}|^2)^{1/2}}d\boldsymbol{y}$.
