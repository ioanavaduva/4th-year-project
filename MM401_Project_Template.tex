...
We obtain a similar formula with $h$ in the place of $g$, and replacing into (\ref{solneven}) we obtain:
...
Since $\gamma_{n+1}=1 \cdot 3 \cdot ... \cdot (n-1)$ and $\alpha(n)=\frac{\pi^{\frac{n}{2}}}{\Gamma(\frac{n+2}{2})}$, then $\gamma_n=2 \cdot 4 \cdot ... \cdot(n-2) \cdot n$. 
Hence, we obtain the general solution in even dimensions:
...
where $n$ is even, $\gamma_n=2 \cdot 4 \cdot ... \cdot(n-2) \cdot n$, $\boldsymbol{x}\in \mathbb{R}^n$ and $t>0$. Note that for $n=2$, $\gamma_2 = 2$ and (\ref{solnev}) agrees with 
\emph{Poisson's formula} (\ref{poiss}) for the two-dimensional solution of the wave equation. Hence, (\ref{solnev}) is a general formula for the even-dimenstional problem.
